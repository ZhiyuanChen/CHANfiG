...
class FlatDict(dict, Mapping[_K, _V]):  # for python 3.7 compatible
    ...
    def interpolate(self, use_variable: bool = True, interpolators: MutableMapping | None = None) -> FlatDict:
        ...
        interpolatees, interpolatee_names = {}, set()
        for key, value in self.all_items():
            interpolatee = find_interpolatees(value)
            if interpolatee:
                for index, name in enumerate(interpolatee):
                    if name.startswith("."):
                        interpolatee[index] = key.rsplit(".", 1)[0] + name
                    if key == name:
                        raise ValueError(f"Cannot interpolate {key} to itself.")
                interpolatees[key] = interpolatee
                interpolatee_names.update(interpolatee)
        circular_references = find_circular_reference(interpolatees)
        if circular_references:
            raise ValueError(f"Circular reference found: {'->'.join(circular_references)}.")
        if use_variable:
            for name in list(interpolatee_names.difference(interpolatees.keys())):
